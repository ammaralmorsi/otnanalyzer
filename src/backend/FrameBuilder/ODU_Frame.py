...
class ODU_Frame:
    ...
    def ODU_TCMi_inner_Overhead_Constructor(self):
        ...
        self.TCMi_keys = [OTN_OH.ODU_TCM1 , OTN_OH.ODU_TCM2 , OTN_OH.ODU_TCM3 , OTN_OH.ODU_TCM4 , OTN_OH.ODU_TCM5 , OTN_OH.ODU_TCM6]
        for i in self.TCMi_keys:
            ODU_TCMi_Data = self.ODU_Overhead_data_mapper[i][2]
            binary_data = bin(int(ODU_TCMi_Data, 16))[2:].zfill(8)
            i.value.inner_levels['BEI_BIAE'] = hex(int(binary_data[0:5], 2))[2:],   # the [2:] to remove the 0x when changed to hex
            ...
            i.value.inner_levels['TTI'] = self.ODU_Overhead_data_mapper[OTN_OH.ODU_PM][0],
            i.value.inner_levels['BIP_8'] = self.ODU_Overhead_data_mapper[OTN_OH.ODU_PM][1]
